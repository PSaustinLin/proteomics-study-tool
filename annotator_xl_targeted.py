...
cid = False
black = True
consider_plus_zero_signature = True
class CrosslinkedMS2Annotator:
    ...
    def calculate_theoretical_crosslinked_fragments(self, precursor_charge: int) -> Tuple[List[float], List[float]]:
        ...
        for charge in range(1, max_charge + 1):
            ...
            b_ion_mz = (non_crosslink_b_mass + charge * self.proton_mass) / charge
            precursor_ions.append((b_ion_mz, '', charge, f'M', sig_type))
    ...
    def annotate_crosslinked_spectrum(self, output_file: str = None, csv_output: str = None, csv_input: str = None):
        # Get spectrum data
        ...
        if csv_input:
            ...
            for _, row in df.iterrows():
                ...
                if intensity >= 1:  # annotation threshold = 1%
                    if position:
                        label = f'{ion_label}_{{{position}}}{str(sig_type)}^{{+{charge}}}'
                    else:
                        label = f'{ion_label}{str(sig_type)}^{{+{charge}}}'
            ...
            # Original code: Calculate theoretical fragments
            ...
            # Create matched_ions list for potential CSV output
            matched_ions = []
        ...
        def calculate_label_positions(annotations, min_spacing=10): # default = 5
            n = len(annotations)
            if n == 0:
                return []
            ...
            # Check for nearby higher peaks and adjust positions accordingly
            for i, ann in enumerate(annotations):
                ann_mz = ann['mz']
                ann_intensity = ann['intensity']
                
                # Find peaks within ±25 m/z range
                nearby_peaks_mask = np.abs(mz_values - ann_mz) <= 25
                nearby_peaks_intensities = normalized_intensities[nearby_peaks_mask]
                
                if len(nearby_peaks_intensities) > 0:
                    max_nearby_intensity = np.max(nearby_peaks_intensities)
                    
                    # If there are higher peaks nearby, elevate the annotation
                    if max_nearby_intensity > ann_intensity:
                        positions[i] = max_nearby_intensity + 5  # 5 units above highest nearby peak
            
            # Iterate through annotations to ensure no vertical overlaps
            for i in range(n):
                for j in range(i):
                    # Check if labels might overlap horizontally
        ...
        for ann, y_pos in zip(all_annotations, label_positions):
            # Check if annotation was elevated due to nearby higher peaks
            original_y_pos = ann['intensity'] + 2
            was_elevated = y_pos > original_y_pos + 3  # threshold for considering it "elevated"
            
            # Plot peak label
            ax_spectrum.text(ann['mz'], y_pos + 1.0, #default 0
                            ann['label'],
                            ha='center',
                            va='bottom',
                            color=ann['color'],
            ...
            # Draw connection line if label was elevated due to nearby higher peaks
            if was_elevated:
            ...
            # Draw connection line if label was elevated due to nearby higher peaks
            if was_elevated:
                # Find the nearest other annotation by m/z distance
                other_mzs = [other['mz'] for other in all_annotations if other is not ann]
                if other_mzs:
                    nearest_mz = min(other_mzs, key=lambda m: abs(m - ann['mz']))
                    # Bend away from the nearest signal: if nearest is to the right, bend left (negative offset), and vice versa
                    bend_direction = -1 if nearest_mz > ann['mz'] else 1
                else:
                    bend_direction = 1  # default: bend right if no neighbors

                # 45-degree line: horizontal offset equals vertical rise
                vertical_rise = y_pos - 1 - ann['intensity']
                x_offset = bend_direction * abs(vertical_rise)  # 45 degrees
                x_start = ann['mz']
                x_end = ann['mz'] + x_offset

                ax_spectrum.plot([x_start, x_end],
                            [ann['intensity'], y_pos - 1],
                            color='grey',
                            linestyle=':',
                            linewidth=1)
